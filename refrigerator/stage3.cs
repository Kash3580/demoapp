...
class Program
{
    ...
    static void Main()
    {
        while (true)
        {
            Console.WriteLine("Enter a command: Insert, Consumption, Current Status, History, Check Expiry, Generate Shopping List, Exit");
    ...
    static void GenerateShoppingList()
    {
        ...
        Console.WriteLine("Generating Shopping List...");

        foreach (var product in products)
        {
            if (product.Value.Quantity <= 0)
            {
                shoppingList.Add(product.Key);
                Console.WriteLine($"{product.Key} added to the shopping list.");
            }
        }

        if (shoppingList.Count == 0)
        {
            Console.WriteLine("No items need to be added to the shopping list.");
        }
    }
}
