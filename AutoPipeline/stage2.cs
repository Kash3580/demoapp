...
class Program
{
    ...
    static void ExecuteCommand(string commandName, string[] commandParameters)
    {
        switch (commandName)
        {
            ...
            case "Execute Command File":
                string commandFile = commandParameters[0];
                string[] subCommands = File.ReadAllLines(commandFile);
                foreach (string subCommand in subCommands)
                {
                    string[] subCommandParts = subCommand.Split(';');
                    string subCommandName = subCommandParts[0];
                    string[]
